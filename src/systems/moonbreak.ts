...
export function createMoonbreakVoice(bus: SynthesisBus): MoonbreakVoice {
  ...
  safety.threshold.value = -16
  safety.knee.value = 18
  safety.ratio.value = 3.2
  ...
  out.gain.setTargetAtTime(0.88, born, 0.38)
  ...
  return {
    set(state) {
      ...
      out.gain.setTargetAtTime(0.88 * paused, now, state.paused ? 0.1 : 0.35)
      ...
      windGain.gain.setTargetAtTime(open * moving * (0.028 + fast * 0.07), now, 0.16)
      ...
      waterGain.gain.setTargetAtTime(
        surface * (0.018 + fast * 0.022) + depth * (0.026 + fast * 0.018),
        now,
        0.28,
      )

      sprayBand.frequency.setTargetAtTime(1700 + fast * 2300, now, 0.18)
      sprayGain.gain.setTargetAtTime(surface * fast * (0.018 + state.exposed * 0.022), now, 0.14)

      flowLow.frequency.setTargetAtTime(680 + fast * 1250, now, 0.2)
      flowGain.gain.setTargetAtTime(depth * moving * (0.038 + fast * 0.082), now, 0.18)
      pressureLow.frequency.setTargetAtTime(82 + depth * 72 + fast * 25, now, 0.35)
      pressureGain.gain.setTargetAtTime(depth * (0.026 + fast * 0.032), now, 0.3)

      glassBand.frequency.setTargetAtTime(820 + fast * 1350 + state.creature * 160, now, 0.22)
      glassGain.gain.setTargetAtTime(depth * (0.004 + fast * 0.019 + state.creature * 0.026), now, 0.2)
      tubeA.frequency.setTargetAtTime(44 + fast * 11, now, 0.4)
      tubeB.frequency.setTargetAtTime(71 + fast * 17, now, 0.4)
      tubeGain.gain.setTargetAtTime(depth * (0.003 + fast * 0.006 + state.creature * 0.012), now, 0.35)
      verbSend.gain.setTargetAtTime(0.08 + depth * 0.42, now, 0.45)

      const growth = clamp(state.orchard + state.reeds)
      growthBand.frequency.setTargetAtTime(1150 + state.reeds * 1700 + fast * 620, now, 0.18)
      growthGain.gain.setTargetAtTime(surface * growth * moving * (0.018 + fast * 0.036), now, 0.12)

      creatureTone.frequency.setTargetAtTime(34 + fast * 8, now, 0.4)
      creatureGain.gain.setTargetAtTime(depth * clamp(state.creature) * 0.055, now, 0.28)
      ...
      if (forward && lastDepth < 0.09 && depth >= 0.09) {
        ...
        burst(now, 0.12 * force, 0.62, 'lowpass', 430, 0.7, 0.48)
        burst(now + 0.015, 0.085 * force, 0.82, 'highpass', 1050, 0.5, 1.65)
        pitchDrop(now, 112, 43, 0.075 * force, 0.72)
      }
      if (forward && lastDepth < 0.72 && depth >= 0.72) {
        burst(now, 0.045 + fast * 0.035, 0.48, 'bandpass', 720, 2.8, 0.8, verbSend)
        pitchDrop(now, 76, 48, 0.045, 0.58)
      }
      if (forward && lastDepth > 0.12 && depth <= 0.12 && state.s > 1700) {
        burst(now, 0.11 + fast * 0.04, 0.74, 'highpass', 820, 0.45, 1.8)
        burst(now + 0.025, 0.07, 0.4, 'bandpass', 680, 0.8, 0.72)
        pitchDrop(now, 54, 126, 0.052, 0.62)
      }
      ...
      if (forward && depth > 0.38 && speed > 7 && rib !== lastRib) {
        lastRib = rib
        burst(now, 0.018 + fast * 0.026, 0.055, 'bandpass', 780 + fast * 760, 4.5, 1.25, verbSend)
        burst(now, 0.012 + fast * 0.016, 0.07, 'lowpass', 230, 0.85, 0.5)
      } else if (depth <= 0.38) {
        lastRib = rib
      }
    ...
    arch(force) {
      ...
      burst(now, 0.04 + amount * 0.055, 0.19, 'bandpass', 520 + amount * 540, 0.7, 0.92)
      burst(now + 0.035, 0.025 + amount * 0.05, 0.12, 'lowpass', 260, 1.1, 0.55)
      burst(now + 0.02, 0.018 + amount * 0.03, 0.38, 'highpass', 1500, 0.55, 1.9, verbSend)
    },
