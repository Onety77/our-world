...
## 9 Sep · Claude · The Fold, the sixth place

> *"somewhere we go and learn something, mostly alone... i kinda have this idea
> of like adopting a pet in the garden like a dog... when you do, it grows"*

A shallow fold in the hills, full of mist, with an animal on the slope for every
thing either of them is trying to get better at. **The mist is the only progress
indicator and it is weather rather than an interface**: it lies in the fold when
you arrive with work waiting, and doing the work pulls it back off the far side.
What it gives back is every animal on the hill, including the ones nobody has
fed for a month. Full write-up in `src/sections/fold/README.md`.

**It is the first place ever *inserted* into the row rather than appended** —
between the Stars and the Lantern Walk, which moved to `order: 5`. The warning
on the Lantern Walk's own definition is what flagged the risk and it was checked
before anything moved: `HUB_STREAM` is measured off `ANCHORS[1]`, so an
insertion at index four cannot reach it. Two positional arrays had to move in
step (`PLACES` in `world/hub/layout`, `LANDMARKS` in `world/GardenHub`) and
nothing else in the garden knows a place by its number. The wood was widened a
second time — `HUB_WOOD.inner` 80 → 105 — because six landmarks span 138 m and
the camera at the far end stands about 86 m out.

### The one argument with the brief, and why

The ask was for a creature that *shrinks and becomes angry* when you stop.
`data/types.ts` had already settled that for plants — *a garden that punishes a
missed week kills the habit it exists to build* — and this is the same argument
with fur on it. So **growth only ever goes up**, and what absence moves is
*where the animal is standing*: near and awake, or lying down at the far end.

The teeth are in the mist instead. Neglect is not announced, it is out of sight,
and you have to do today's work to see what you have been neglecting. Coming
back after a month and watching the fog pull off four sleeping animals says
everything a red streak counter would, without once telling anybody off.

### Four things that cost time

**The mist did not exist for the first hour and the screenshots looked fine.**
`started` was only written when a *session* began, so before anybody pressed
anything it was 0 — and 0 meant "nothing due", which meant "the hill is clear".
The place opened in full view every time. It is counted on *arrival* now, and
`null` (not yet counted) is deliberately distinguished from `0` (nothing to do),
because collapsing those two clears the hill for the first second of every visit
and then rolls the fog back in on somebody who has work waiting.

**`translate` is spoken for; layout must use `transform`.** `drift-in` animates
the independent `translate` property and ends on `translate: none` with
`fill-mode: both`, which persists — so centring a fixed block with
`translate: -50% 0` is wiped the moment the animation finishes. This is the trap
already in PLAN.md, met from the other side: the fix recorded there moved the
*animation* off `transform`, which leaves `transform` as the property layout may
own. It put the word's centre at x = 369 on a 393-pixel screen and "I had it" at
x = 414, entirely off the picture — and **it is invisible in a screenshot**,
because a left-shifted block of centred text still looks like a block of text.
Only the measured rectangles said so.

**A transformed ancestor is the containing block for `position: fixed`
children.** The three forms were rendered inside `.threshold`, which is fixed
*and* transformed, so `inset: 0` resolved against a 34-rem box near the bottom of
the page: the sheet came out as a small dark panel with half its fields scrolled
out of sight. They are opened from the threshold and rendered by `FoldSitting` at
the top level now — `forming` in `systems/tending` is the whole of that fix.

**Projecting a local position through the camera.** The herd published every
animal as 480 metres away and eleven thousand pixels off the left of the screen —
480 being `FOLD_X`, the group the whole place is drawn in. The picture was
identical either way; only `window.__fold` said so. `localToWorld` now, which is
the same note the Lantern Walk keeps.

Also: the landmark's mist was additive and therefore invisible over a bright
midday meadow (mist is something *in the way*, not light), and then it was
occluded by its own near ridge.

### Two things about `firestore.rules` that are worth more than the section

**`affectedKeys()` is top-level only.** Updating `boxes.warm` changes the
*`boxes` field* — so the affected key is `boxes`, and the seal as first written,
`hasOnly(['boxes.' + me(), …])`, matches nothing and would have refused every
single turn on the real backend. It has to be said in two steps: the document
may touch only those three maps, and then `MapDiff` on each map says that only
your own key inside it moved. The rule reads better for it, and this is the sort
of thing that cannot be found from one device or from the mock.

**And `npm run rules` cannot see a broken rule.** Its self-check counts
`match `, `allow `, `function `, `if ` and braces before and after stripping
comments — none of which changes if the *contents* of a rule are wrong. A shell
quoting accident had emptied every string literal in the new block
(`kind in [words, days]`, `get(note, )`, and the seal itself) and `npm run
rules` printed its usual success. Nothing downstream of that file is compiled,
so it is the one place in this repo where a mangled edit is completely silent —
**read the emitted `rules-out/firestore.rules` after touching the template.**

Two smaller ones from the same cause: the check also refuses any comment
containing the word "if" followed by a space, because it counts the substring
`'if '` and cannot tell prose from a rule. And the door still said *"Five
places… and a glasshouse"* — the first sentence anybody reads, describing a
count that was wrong and a place that had not existed since the 7th. It names
all six now, and `ui/Arrival` carries a note saying that line is part of the job
whenever a place is added, moved or renamed.

### Verified

`npm run fold` — 55 assertions over the spacing, the growth curve, the per-person
seal on a shared word, and the mist's actual fog values at the distances the
animals really stand at. It covers the two things no screenshot can: a schedule
that plays out over five weeks, and a seal that cannot be seen from one device.

Real CDP touch at 393×852 and 1280×800 (`scratchpad/fold-flow.mjs`), through the
whole loop: threshold → six words → the hill opening → "that is all of them",
plus taking an animal in, putting a word in, handing one over, and marking a day.

| | |
|---|---|
| threshold | `6 words ready · 2 to say you did` |
| mist on arrival | near 6 m, far 46 m — a kept animal at 23 m is 39% fogged |
| after five words | progress 0.63, near 20.7, far 91.7 |
| through | `that is all of them`, progress 0.75 — the two unmarked days remain |
| the Fold chunk | 12.2 kB (5.1 kB gzipped), lazy like every other place |

What it costs, off `window.__frame` at 393×852 rather than off `npm run tris`,
which only counts the garden and still hardcodes 150 trees:

| | triangles | draw calls |
|---|---|---|
| the Fold | 187,633 | 41 |
| the Lantern Walk, for scale | 86,912 | 12 |
| the garden | 728,978 | 50 |

Twice the Lantern Walk and a quarter of the garden; most of it is the near grass,
which is the half of the frame you actually stand in. Forty-one calls against a
budget in the hundreds.

**And the garden itself got about 5% heavier, which is mine.** Widening the wood
for a sixth place meant a quarter more ground, and holding the same 170 trees
over it would have thinned the treeline visibly — the backdrop the whole garden
is read against. Two hundred trees is roughly +36k triangles on the hub. The
alternative was tightening the gaps between places, which `world/hub/layout` has
a standing note against.

`npm run shaders` (all 104), typecheck and the production build pass.

### What is not done

`npm run places` has the new id but has not been re-run, so the Fold has no
measured soundscape. Nothing here has been seen on a real GPU — only SwiftShader.
The garden landmark is adequate rather than good: the mist band reads as haze
rather than as a distinct bank. Nothing tests the `firestore.rules` block for
`practices` and `words` against a live Firestore — same standing as the archive's
seal and the hearts rule. And `speak()` in `systems/fold` is a named, empty seam:
pronunciation wants a Cloud Function, because an OpenAI key in a `VITE_` variable
ships inside the bundle Vercel serves before anybody signs in.
