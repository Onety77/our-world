...
- [x] **The garden keeps itself on the device.** Firestore holds a persistent
      local cache, so opening is a read off the disk rather than a round trip
      to another continent, and a thought written with no signal queues and
      sends itself. A service worker (`sw/worker.js`, generated by
      `gardenWorker()` in `vite.config.ts` from the entry's *static* import
      graph only) precaches the 42-file shell, so the world opens instantly and
      opens with no network at all. Media is deliberately never cached — a
      ranged request answered from a whole body is a stall with no error on it.
      An update is **offered**, never taken: `ui/Renewal` is one line on the
      world and nothing reloads until it is touched. `npm run offline` builds,
      serves, opens it, **kills the server** and asks for the world again.
      **The push worker moved to `public/push/` and must stay there** — two
      workers cannot share a scope, and at the root it would silently replace
      the cache
- [x] **Coming back where you were.** `systems/whereYouWere` writes the place
      down as you move and reads it back on the way in, so a phone that
      discards the world while you answer a message gives it back instead of
      starting it over. It forgets after an hour: a resume is for an
      interruption, not for a visit. The door still opens
- [x] **Five things the browser already had.** The screen stays lit during a
      race and a film (it dimming mid-race was a defect, not a gap); the count
      `Whisper` already computes now also sits on the home-screen icon;
      vibration on the release, on impacts weighed by how hard, and on the
      line; the orientation is held *whichever way the phone is already being
      held*, because this world is portrait-first and forcing landscape would
      override a design decision rather than protect one; and a **share
      target**, so a photograph goes from her camera roll into the Glasshouse
      without the garden being opened first
- [x] **A thought can be sealed until a day.** Left for a birthday, an
      anniversary, the morning she lands. The words live in
      `letters/{id}/sealed/words` and `firestore.rules` refuses them until the
      day, so what her device receives is a **gap** rather than something
      censored on the way past — the question vine's shape, and the archive's.
